...
// Admin client uses the migration/admin role to set up validation data and read logs.
const adminPrisma = new PrismaClient();

// App client connects as nexio_app non-superuser (to verify RLS enforcement)
...
async function main() {
  ...
  // 1. Get tenants list using admin client (the runtime role has no tenant context yet)
  ...
  // 2. Check if RLS blocks access for nexio_app when no tenant context is set
  ...
  auditLogs.forEach((log) => {
    console.log(
      `- Action: ${log.action}, Table: ${log.tableName}, User: ${log.userId}, IP: ${log.clientIp}, UA: ${log.userAgent}\n  Old: ${JSON.stringify(log.oldValues)}\n  New: ${JSON.stringify(log.newValues)}`,
    );
  });
