...
export class ConfigService {
  ...
  get jwtSecret(): string {
    return this.nestConfigService.get<string>('JWT_SECRET', 'nexio_secret_access_key_change_me_in_production');
  }

  get jwtRefreshSecret(): string {
    return this.nestConfigService.get<string>('JWT_REFRESH_SECRET', 'nexio_secret_refresh_key_change_me_in_production');
  }
  ...
  validateSecrets(): void {
    ...
    const hasDefaultSecret = !jwtSecretVal || jwtSecretVal === defaultSecret || this.jwtSecret === defaultSecret;
    const hasDefaultRefresh = !jwtRefreshSecretVal || jwtRefreshSecretVal === defaultRefreshSecret || this.jwtRefreshSecret === defaultRefreshSecret;

    if (isProd && (hasDefaultSecret || hasDefaultRefresh)) {
      throw new Error('FATAL STARTUP EXCEPTION: Production environment cannot use default JWT_SECRET or JWT_REFRESH_SECRET');
    }
    if (this.nestConfigService.get<string>('STRICT_SECURITY') === 'true' && (hasDefaultSecret || hasDefaultRefresh)) {
      throw new Error('FATAL STARTUP EXCEPTION: Default JWT secrets forbidden under STRICT_SECURITY');
    }
  }
