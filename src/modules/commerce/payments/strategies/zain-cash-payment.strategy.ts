...
@Injectable()
export class ZainCashPaymentStrategy implements PaymentGatewayStrategy {
  readonly providerName = 'zaincash';
  readonly aliases = ['zaincash', 'zain-cash', 'zain'];

  async createPayment(amount: number, currency: string, orderId: string, metadata?: Record<string, unknown>): Promise<PaymentResponse> {
    const paymentReference = `ZC-${orderId.slice(0, 8)}-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
    return {
      success: true,
      paymentReference,
      externalTransactionId: `TX-ZC-${Date.now()}`,
      status: 'PENDING',
      rawResponse: { provider: 'zaincash', orderId, amount, currency, metadata },
    };
  }
  ...
  async verifyPayment(paymentReference: string, externalTransactionId?: string): Promise<PaymentResponse> {
    return {
      success: true,
      paymentReference,
      externalTransactionId: externalTransactionId || `TX-ZC-${Date.now()}`,
      status: 'PAID',
    };
  }
  ...
  async refundPayment(paymentReference: string, amount: number, reason?: string): Promise<PaymentResponse> {
    return {
      success: true,
      paymentReference,
      externalTransactionId: `REF-ZC-${Date.now()}`,
      status: 'REFUNDED',
