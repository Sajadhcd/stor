...
@Injectable()
export class QiCardPaymentStrategy implements PaymentGatewayStrategy {
  readonly providerName = 'qicard';
  readonly aliases = ['qicard', 'qi-card', 'qi'];

  async createPayment(amount: number, currency: string, orderId: string, metadata?: Record<string, unknown>): Promise<PaymentResponse> {
    const paymentReference = `QI-${orderId.slice(0, 8)}-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
    return {
      success: true,
      paymentReference,
      externalTransactionId: `TX-QI-${Date.now()}`,
      status: 'PENDING',
      rawResponse: { provider: 'qicard', orderId, amount, currency, metadata },
    };
  }
  ...
  async verifyPayment(paymentReference: string, externalTransactionId?: string): Promise<PaymentResponse> {
    return {
      success: true,
      paymentReference,
      externalTransactionId: externalTransactionId || `TX-QI-${Date.now()}`,
      status: 'PAID',
    };
  }
  ...
  async refundPayment(paymentReference: string, amount: number, reason?: string): Promise<PaymentResponse> {
    return {
      success: true,
      paymentReference,
      externalTransactionId: `REF-QI-${Date.now()}`,
      status: 'REFUNDED',
